...
export class PaidAiAdapter extends BaseBillingAdapter {
  ...
  async validateApiKey(): Promise<boolean> {
    try {
      ...
      // For other errors, assume key might be valid but service is unavailable
      throw error;
    }
  }
}
...
export function createPaidAiAdapter(
  ...
  return new PaidAiAdapter(tenantId, mode, {
    apiKey: config.apiKey,
    baseUrl: config.baseUrl || 'https://api.paid.ai/v1',
  });
}
