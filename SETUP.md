...
> **Important**: This repository uses Git submodules. When cloning, use `git clone --recurse-submodules` or run `git submodule update --init --remote` after cloning to initialize the `/shadcn-ui` submodule which contains reference UI components.
