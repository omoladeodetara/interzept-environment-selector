...
- **Pluggable Storage**: Use in-memory for development, inject database adapter for production
- **Statistical Analysis**: Conversion rate and ARPU calculations
- **Async API**: All functions are async-ready for database operations
...
const variant = await elo.assignVariant('user123', 'experiment_001');

// Track a conversion
await elo.trackConversion('user123', 'experiment_001', {
  revenue: 39.99,
  timestamp: new Date()
});

// Get experiment results
const results = await elo.getExperimentResults('experiment_001');
```

## Storage Adapters

By default, Elo uses in-memory storage. For production, implement the `StorageAdapter` interface:

```typescript
import { StorageAdapter, setStorageAdapter } from '@packages/elo';
...
// Set custom adapter at application startup
setStorageAdapter(new PostgresStorageAdapter());
```
...
Inject a custom storage adapter for production use.
...
- Implement a database-backed `StorageAdapter` (PostgreSQL, Redis, etc.)
