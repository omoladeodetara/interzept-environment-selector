...
import { calculateStatisticalSignificance } from './index';
...
export function createExperiment(
  ...
  const variants: ExperimentVariant[] = request.variants.map((v) => ({
    id: uuidv4(),
    name: v.name,
    price: v.price,
...
export function getExperimentResults(
  ...
  // Validate experiment exists and tenant has access
  ...
  if (variantList.length >= 2) {
    ...
    const result = calculateStatisticalSignificance(sorted[1], sorted[0]);
    statisticalSignificance = 1 - result.pValue;
  }
